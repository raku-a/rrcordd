...
// ── User accounts (username -> { salt, hash, color }) ─────────────────────────
...
function registerUser(username, password) {
  ...
  accounts.set(username.toLowerCase(), { username, salt, hash, color });
  return color;
}
...
function loginUser(username, password) {
  ...
  return { ok: true, color: acc.color, username: acc.username };
}
...
wss.on('connection', (socket) => {
  ...
  socket.on('message', (raw) => {
    ...
    if (msg.type === 'register') {
      ...
      const color = registerUser(username, password);
      info = { id: clientId, username, color };
      clients.set(socket, info);

      send(socket, { type: 'auth_ok', username, color });
    ...
    if (msg.type === 'login') {
      ...
      info = { id: clientId, username: result.username, color: result.color };
      clients.set(socket, info);

      send(socket, { type: 'auth_ok', username: result.username, color: result.color });
      send(socket, { type: 'history', messages });
